...
t_gnode::_process_table(t_uindex port_id) {
    ...
    result.m_should_notify_remove = false;
    ...
    _add_all_computed_columns(
        _process_state.m_transitions_data_table, DTYPE_UINT8);

    // Recompute values for flattened and m_state->get_table
    _recompute_all_columns(get_table_sptr(),
        _process_state.m_flattened_data_table, _process_state.m_lookup);
    ...
    _compute_all_columns({_process_state.m_delta_data_table,
        _process_state.m_prev_data_table, _process_state.m_current_data_table});
    ...
    column_names.insert(column_names.end(), valid_computed_columns.begin(),
    ...
    tbb::parallel_for(0, int(ncols), 1,
        [&_process_state, &column_names, &result, this](int colidx)
#else
    ...
    if (existed_mask.count() == _process_state.m_flattened_data_table->size()) {
        ...
        result.m_should_notify_remove = true;
    } else {
        flattened_masked
...
t_gnode::_register_context(
    ...
    switch (type) {
        case TWO_SIDED_CONTEXT: {
            ...
            if (should_update) {
                // Compute all valid computed columns + new computed columns
                // that were added as part of this context. Do so separately
                // from update_context_from_state, so that registration-specific
...
t_gnode::_unregister_context(const std::string& name) {
    ...
    switch (type) {
        ...
        case TWO_SIDED_CONTEXT: {
            ...
            m_computed_column_map.remove_computed_columns(
...
t_gnode::notify_contexts(const t_data_table& flattened) {
    ...
    for (std::map<std::string, t_ctx_handle>::const_iterator iter
         = m_contexts.begin();
...
void
t_gnode::_recompute_all_columns(std::shared_ptr<t_data_table> tbl,
...
t_gnode::_add_computed_column(
    ...
    if (computation.m_name == INVALID_COMPUTED_FUNCTION) {
        std::cerr << "Cannot add computed column `" << computed_column_name
                  << "` in gnode." << std::endl;
        return;
    }
    ...
    tbl->add_column_sptr(computed_column_name, output_column_type, true);
}

void
t_gnode::_compute_column(const t_computed_column_definition& computed_column,
    std::shared_ptr<t_data_table> tbl) {
    std::string computed_column_name = std::get<0>(computed_column);
    std::vector<std::string> input_column_names = std::get<2>(computed_column);
    t_computation computation = std::get<3>(computed_column);
    ...
    if (computation.m_name == INVALID_COMPUTED_FUNCTION) {
        std::cerr << "Cannot compute column `" << computed_column_name
                  << "` in gnode." << std::endl;
        return;
    }
    ...
    t_computed_column::apply_computation(
        input_columns, output_column, computation);
}

void
t_gnode::_recompute_column(const t_computed_column_definition& computed_column,
    ...
    if (computation.m_name == INVALID_COMPUTED_FUNCTION) {
        std::cerr << "Cannot re-compute column `" << computed_column_name
                  << "` in gnode." << std::endl;
        return;
    }
    ...
    t_computed_column::reapply_computation(table_columns, flattened_columns,
        changed_rows, output_column, computation);
}
